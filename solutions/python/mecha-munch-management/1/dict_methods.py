...
def send_to_store(cart, aisle_mapping):
    ...
    for key in reversed(sorted(cart.keys())):
        #storing cart quantity before merging
        cart_quantity = [cart[key]]
        ful_dict.setdefault(key, cart_quantity + aisle_mapping[key])
...
def update_store_inventory(fulfillment_cart, store_inventory):
    ...
    for key in fulfillment_cart.keys():
        ...
        if store_inventory.get(key, 0) == 0:
            continue
        #listing inventory
